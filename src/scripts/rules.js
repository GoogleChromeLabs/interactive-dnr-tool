...
function getHighestPriorityRules(matchedRulesList) {
  ...
  const highestPriorityAction = Math.min(
    ...highestPriorityRules.map((rule) => actionByPriority.indexOf(rule.rule.action.type))
  )
  ...
  if (
    ...
    let allowRules = highestPriorityRules.filter(
      (rule) => rule.rule.action.type === 'allow' || rule.rule.action.type === 'allowAllRequests'
    )
    ...
    const highestModifyHeadersPriority = Math.min(
